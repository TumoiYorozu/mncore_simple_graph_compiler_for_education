...
class AddOperator(BaseOperator):
    ...
    def generate_vsm(self) -> List[str]:
        ...
        if shape0 == [256, 16] and shape1 == [16]:  # add_colvec（列ベクトル加算）
            self.check_layout_in(
                "((8_L2B:1, 4:2, 8_L1B:1), (2:1, 4_PE:1, 2_W:1))", 0)
            self.check_layout_in("((2:1, 4_PE:1, 2_W:1))", 1)
            self.check_layout_out(
                "((8_L2B:1, 4:2, 8_L1B:1), (2:1, 4_PE:1, 2_W:1))")
    ...
    def generate_cpp(self) -> List[str]:
        ...
        if len(shape0) == 2 and len(shape1) == 1:
            # バイアス加算のケース (2D + 1D)
            assert shape0[1] == shape1[0], f"Dimension mismatch: shape0[1]={
                shape0[1]} != shape1[0]={shape1[0]}"
            lines.append(f"    const Matrix<{shape0[0]}, {shape0[1]}> {
                         out_var} = add_colvec<{shape0[0]}, {shape0[1]}>({in0}, {in1});")
        else:
            raise NotImplementedError(f"Add: shapes {shape0} + {shape1}")
